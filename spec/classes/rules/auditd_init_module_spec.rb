      ...
      context "on #{os} with enforce = #{enforce} and arch = #{os_facts[:os]['architecture']}" do
        ...
        it {
          ...
          if enforce
            ...
            is_expected.not_to contain_concat__fragment('watch init_module command rule 1')
            is_expected.not_to contain_concat__fragment('watch init_module command rule 2')
          end
        }
      end
    end
