...
class cis_security_hardening::rules::auditd_perm_mod (
  ...
  if $enforce {
    ...
    concat::fragment { 'watch perm mod rule 1':
      ...
      content => "-a always,exit -F arch=b32 -S chmod,fchmod,fchmodat -F auid>=${uid} -F auid!=${auid} -k perm_mod",
    }
    concat::fragment { 'watch perm mod rule 2':
      order   => '92',
      target  => $cis_security_hardening::rules::auditd_init::rules_file,
      content => "-a always,exit -F arch=b32 -S chown,fchown,fchownat,lchown -F auid>=${uid} -F auid!=${auid} -k perm_mod",
    }
    concat::fragment { 'watch perm mod rule 3':
      order   => '93',
      target  => $cis_security_hardening::rules::auditd_init::rules_file,
      content => "-a always,exit -F arch=b32 -S setxattr,lsetxattr,fsetxattr,removexattr,lremovexattr,fremovexattr -F auid>=${uid} -F auid!=${auid} -k perm_mod", #lint:ignore:140chars
    }
    if $facts['architecture'] == 'x86_64' or $facts['architecture'] == 'amd64' {
      concat::fragment { 'watch perm mod rule 4':
        order   => '94',
        target  => $cis_security_hardening::rules::auditd_init::rules_file,
        content => "-a always,exit -F arch=b64 -S chmod,fchmod,fchmodat -F auid>=${uid} -F auid!=${auid} -k perm_mod",
      }
      concat::fragment { 'watch perm mod rule 5':
        order   => '95',
        target  => $cis_security_hardening::rules::auditd_init::rules_file,
        content => "-a always,exit -F arch=b64 -S chown,fchown,fchownat,lchown -F auid>=${uid} -F auid!=${auid} -k perm_mod",
      }
      concat::fragment { 'watch perm mod rule 6':
        order   => '96',
        target  => $cis_security_hardening::rules::auditd_init::rules_file,
        content => "-a always,exit -F arch=b64 -S setxattr,lsetxattr,fsetxattr,removexattr,lremovexattr,fremovexattr -F auid>=${uid} -F auid!=${auid} -k perm_mod", #lint:ignore:140chars
      }
    }
  }
}
