...
class cis_security_hardening::rules::auditd_pam_timestamp_check_use (
  ...
  if $enforce {
    concat::fragment { 'watch pam_timestamp_check command rule 1':
      ...
      content => '-a always,exit -F path=/usr/sbin/pam_timestamp_check -F perm=x -F auid>=1000 -F auid!=4294967295 -k privileged-pam_timestamp_check', #lint:ignore:140chars
    }
  }
}
